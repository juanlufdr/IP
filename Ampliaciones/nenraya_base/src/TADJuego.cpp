...
void pintarTablero(Juego J) {
	int i, j, color;
	for (i = 0; i < numeroFilas(J.t); i++) {
		for (j = 0; j < numeroColumnas(J.t); j++) {
			if (esVaciaCasilla(J.t.tab[i][j])) {
				TEntornoEliminarCasilla(i, j);
			} else {
				color = queColorCasilla(J.t.tab[i][j]);
				TEntornoPonerCasilla(i, j, color);
			}
		}
	}
}
